...
function initRequest() {
    return new Promise(function(resolve, reject) {
        ...
        if (fs.existsSync(output)) {
            ...
            if (!fileStats.isFile()) {
                throw new Error('output file is already exists and it is a directory');
            } else {
                // resume downloads
    ...
    var curRequestOnFinishedPromise = new Promise((resolve, reject) => {
        ...
        var request = requestHandler.request(requestOptions, (response) => {
            ...
            response.on('error', (error) => {
                ...
                return reject(new Error(fmt.getMessage('connection (%02d): an error occurred: %s', curConnectionId, error.toString())));
            });
...
function printProgressBar() {
    var i, conn = null,
        ...
        progress, totalContentLength, downloadedLength = 0;
    ...
    for (i = 1; i <= options.numConnections; i++) {
        ...
        str = '';
        totalContentLength = conn.end - conn.start + 1;
        progress = conn.curContentLength / totalContentLength;
        ...
        if (options.verbose) {
            str += fmt.getMessage('] (%.2f%%) (start=%d,end=%d,total=%d,current=%d) %s', parseFloat(progress * 100).toFixed(2), conn.start, conn.end, totalContentLength, conn.curContentLength, getSpeedStr(conn.curContentLength));
        } else {
            str += fmt.getMessage('] (%.2f%%) %s', parseFloat(progress * 100).toFixed(2), getSpeedStr(conn.curContentLength));
        }

        if (conn.curContentLength == totalContentLength) {
    ...
    strs.unshift(fmt.getMessage('progress[time useds:%s] %s', clc.green, getTimeUsedStr(), getSpeedStr(downloadedLength)));
    strs.forEach((str) => {
